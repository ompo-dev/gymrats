...
import { paymentsQuerySchema, addPaymentMethodSchema } from "@gymrats/contracts";
import {
  badRequestResponse,
...
export async function getPaymentMethodsHandler({
  ...
  try {
    ...
    return internalErrorResponse(set, "Erro ao buscar métodos de pagamento", error);
  }
}
...
export async function addPaymentMethodHandler({
  ...
  try {
    ...
    return internalErrorResponse(set, "Erro ao criar método de pagamento", error);
  }
}
...
export async function getMembershipsHandler({
  ...
  try {
    ...
    const formattedMemberships = memberships.map((membership) => ({
      ...
      planName: membership.plan?.name || "Plano básico",
