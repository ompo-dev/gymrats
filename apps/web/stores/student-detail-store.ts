...
interface StudentDetailState {
  ...
  getNutritionLibraryPlanDetail: (params: {
    scope: StudentDetailScope;
    studentId: string;
    planId: string;
  }) => Promise<NutritionPlanData | null>;
...
function getNutritionActivateBase(
  scope: StudentDetailScope,
  studentId: string,
) {
...
export const useStudentDetailStore = create<StudentDetailState>()(
  (set, get) => ({
    weeklyPlans: {},
    ...
    updateWorkout: async ({ scope, studentId, workoutId, payload }) => {
      ...
      try {
        await apiClient.put(
          `${getWorkoutManageBase(scope, studentId)}/${workoutId}`,
          payload,
        );
        await get().loadWeeklyPlan(scope, studentId);
      } finally {
    ...
    updateWorkoutExercise: async ({
      ...
      try {
        await apiClient.put(
          `${getWorkoutExercisesBase(scope, studentId)}/${exerciseId}`,
          payload,
        );
        await get().loadWeeklyPlan(scope, studentId);
      } finally {
    ...
    saveNutrition: async ({
      ...
      const nextNutrition = normalizeDailyNutrition(
        {
          ...
          sourceNutritionPlanId:
            previousNutrition?.sourceNutritionPlanId ?? null,
      ...
      const flushPromise = new Promise<void>((resolve, reject) => {
        const resolvers =
          pendingDetailDailyNutritionResolvers.get(requestKey) ?? [];
      ...
      const scheduleFlush = () => {
        const existingTimer =
          pendingDetailDailyNutritionFlushTimers.get(requestKey);
        if (existingTimer) {
        ...
        const timer = setTimeout(() => {
          ...
          const request = apiClient
            .post<{
              ...
              syncPlan:
                latestDetailDailyNutritionSyncPlanFlags.get(requestKey) ??
                false,
            })
            .then((response) => {
              const latestVersion =
                latestDetailDailyNutritionVersions.get(requestKey) ??
                flushVersion;
            ...
            .catch((error) => {
              ...
              settleDetailDailyNutritionResolvers(
                requestKey,
                flushVersion,
                error,
              );
            ...
            .finally(() => {
              if (
                pendingDetailDailyNutritionSyncs.get(requestKey) === request
              ) {
    ...
    getNutritionLibraryPlanDetail: async ({ scope, studentId, planId }) => {
      const response = await apiClient.get<{
        data?: NutritionPlanData | null;
    ...
    updateNutritionLibraryPlan: async ({
      ...
      const previousNutrition =
        get().nutritionByDate[key]?.[currentDate] ?? null;
      const targetPlan =
    ...
    activateNutritionLibraryPlan: async ({ scope, studentId, planId }) => {
      ...
      const sourcePlan =
        (get().nutritionLibraryPlans[key] ?? []).find(
          (plan) => plan.id === planId,
        ) ?? null;
          ...
          nutritionByDate: {
            ...
            [key]: {
              ...
              [currentDate]: createDailyNutritionFromPlan({
                plan: activatedPlan,
                baseNutrition:
                  state.nutritionByDate[key]?.[currentDate] ?? null,
                date: currentDate,
              }),
            },
          },
