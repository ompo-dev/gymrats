...
- Caminho: `app/api/workouts/exercises/[id]`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/workouts/exercises/[id]`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/workouts/exercises/[id]` com método(s) `PUT, DELETE`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `PUT`, `updateExerciseHandler`, `DELETE`, `deleteExerciseHandler`), serializando resposta HTTP.
