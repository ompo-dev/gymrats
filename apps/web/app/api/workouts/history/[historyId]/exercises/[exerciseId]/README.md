...
- Caminho: `app/api/workouts/history/[historyId]/exercises/[exerciseId]`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/workouts/history/[historyId]/exercises/[exerciseId]`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/workouts/history/[historyId]/exercises/[exerciseId]` com método(s) `PUT, PATCH`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `PUT`, `resolve`, `updateExerciseLogHandler`, `PATCH`), serializando resposta HTTP.
