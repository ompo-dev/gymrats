...
- Caminho: `app/api/workouts/[id]/complete`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/workouts/[id]/complete`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/workouts/[id]/complete` com método(s) `POST`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `POST`, `resolve`, `completeWorkoutHandler`), serializando resposta HTTP.
