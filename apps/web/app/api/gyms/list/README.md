...
### `route.ts`
- O que faz: implementa o endpoint `/api/gyms/list` com método(s) `GET`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `GET`, `listGymsHandler`), serializando resposta HTTP.
