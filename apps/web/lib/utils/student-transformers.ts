...
export function transformStudentData(
  apiData: Record<string, string | number | boolean | object | null>,
): Partial<StudentData> {
  ...
  if (apiData.weightHistory) {
    transformed.weightHistory = (
      apiData.weightHistory as Array<{
        date?: string | Date;
        weight?: number;
        notes?: string;
      }>
  ...
  if (apiData.units) {
    transformed.units = (
      apiData.units as Array<
        Record<string, string | number | boolean | object | null>
      >
    ).map((unit) => transformUnit(unit));
  }

  // Transformar workoutHistory
  if (apiData.workoutHistory) {
    transformed.workoutHistory = (
      apiData.workoutHistory as Array<
        Record<string, string | number | boolean | object | null>
      >
    ).map((wh) => transformWorkoutHistory(wh));
  }

  // Transformar personalRecords
  if (apiData.personalRecords) {
    transformed.personalRecords = (
      apiData.personalRecords as Array<
        Record<string, string | number | boolean | object | null>
      >
    ).map((pr) => transformPersonalRecord(pr));
  }
...
function transformProgress(
  progress: Record<string, string | number | boolean | object | null>,
): UserProgress {
  return {
...
function transformUnit(
  unit: Record<string, string | number | boolean | object | null>,
): Unit {
  return {
    id: unit.id,
    title: unit.title,
...
function transformWorkoutHistory(
  wh: Record<string, string | number | boolean | object | null>,
): WorkoutHistory {
  return {
    date: wh.date ? new Date(wh.date) : new Date(),
...
function transformPersonalRecord(
  pr: Record<string, string | number | boolean | object | null>,
): PersonalRecord {
  return {
...
function transformDailyNutrition(
  nutrition: Record<string, string | number | boolean | object | null>,
): DailyNutrition {
  return {
...
function transformSubscription(
  subscription: Record<string, string | number | boolean | object | null>,
...
function transformActiveWorkout(
  workout: Record<string, string | number | boolean | object | null>,
): StudentData["activeWorkout"] {
  return {
