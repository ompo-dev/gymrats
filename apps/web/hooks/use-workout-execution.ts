...
export function useWorkoutExecution() {
  ...
  const workoutProgress =
    totalExercises > 0
      ? Math.min(100, Math.round((totalSeen / totalExercises) * 100))
      : 0;
  ...
  useEffect(() => {
    const init = async () => {
      ...
      console.log(
        `[useWorkoutExecution] Initializing workout: ${openWorkoutId}`,
      );
  ...
  const handleSelectAlternative = useCallback(
    async (exerciseId: string, alternativeId?: string) => {
  ...
  return {
    ...
    handlers: {
      ...
      handleViewEducation: (exerciseId?: string) => {
        ...
        if (ex) {
          const slug =
            ex.educationSlug || ex.name.toLowerCase().replace(/\s+/g, "-");
    ...
    actions: {
      ...
      calculateWorkoutStats,
      setCardioPreference: (
        pref: "before" | "after" | "none",
        duration?: number,
      ) => {
        const { setCardioPreference: storeSetCardioPref } =
    ...
    methods: {
      getCurrentExerciseName: () => {
        ...
        if (altId && currentExercise.alternatives) {
          const alt = currentExercise.alternatives.find(
            (a: AlternativeExercise) => a.id === altId,
          );
...
function createCardioExercises(
  duration: number,
  type?: string,
): WorkoutExercise[] {
  const cardioTypes: Record<
    string,
    {
      name: string;
      icon: string;
