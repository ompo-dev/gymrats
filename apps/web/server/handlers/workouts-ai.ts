  ...
  try {
    ...
    switch (parsedPlan.action) {
          ...
          try {
            ...
            const workout = await db.workout.create({
              data: {
                ...
                type: (workoutPlan.type ?? "strength") as
                  | "strength"
                  | "cardio"
                  | "flexibility",
                muscleGroup: workoutPlan.muscleGroup ?? "full-body",
                difficulty: (workoutPlan.difficulty ?? "intermediario") as
                  | "iniciante"
                  | "intermediario"
                  | "avancado",
                estimatedTime: 0,
            ...
            const planExercises = (workoutPlan.exercises ?? [])
              .filter(
                (e): e is ParsedWorkoutPlanExercise & { name: string } =>
                  !!e?.name,
              )
              .map((e) => ({
                name: e.name!,
            ...
            results.created.push(
              `Workout: ${workoutPlan.title ?? workoutPlan.name ?? "Treino"}`,
            );
            results.created.push(
        ...
        if (workouts.length > 1) {
          ...
          if (workout) {
            ...
            if (oldExercise && firstWorkoutExercises.length > 0) {
              ...
              const newExercisePlan = newEx?.name
                ? {
                    name: newEx.name,
                    sets: newEx.sets,
                    reps: newEx.reps,
      ...
      case "update_workout": {
        if (workouts.length > 1) {
        ...
        } else if (parsedPlan.targetWorkoutId && workouts.length > 0) {
          const workoutPlan = workouts[0];
          ...
          if (!targetWorkout && parsedPlan.targetWorkoutId) {
            ...
            targetWorkout = unit.workouts.find(
...
export async function chatWorkoutsHandler({
  ...
  try {
    ...
    const workoutsInfo = unit.workouts.map(
      (w: {
        id: string;
        title: string;
        type: string;
    ...
    let parsed: {
      ...
      workouts?: Array<{
        title?: string;
        exercises?: Array<Record<string, JsonValue>>;
      }>;
      message?: string;
    } | null = null;
    const tryParseImportedWorkout = (
      raw: Record<string, JsonValue> | JsonValue[],
    ) => {
      ...
      const normalizeWorkout = (w: Record<string, JsonValue>) => ({
        ...
        exercises: normalizeExercises(
          Array.isArray(w.exercises)
            ? (w.exercises as Array<Record<string, JsonValue>>)
            : [],
        ),
      });
    ...
    try {
      if (message.trim().startsWith("{") || message.trim().startsWith("[")) {
        const rawJson = JSON.parse(message) as
    ...
    async start(controller) {
      ...
      try {
        ...
        if (workoutsInfo.length > 0) {
          ...
          const previews = Array.isArray(previewWorkouts)
            ? (previewWorkouts as Array<Record<string, JsonValue>>)
            : [];
          const ref =
            reference &&
...
- CRÍTICO: Retorne TODOS os ${previews.length} workouts no array "workouts"
...
- NÃO crie novos workouts, apenas ATUALIZE o referenciado`;
            } else if (ref.type === "exercise" && ref.exerciseName) {
...
- Você DEVE usar targetWorkoutId="${workoutIdentifier}"`;
        ...
        if (!parsed) {
          ...
          const streamHistory = Array.isArray(conversationHistory)
            ? conversationHistory
            : [];
          const messagesArr: Array<{
            role: "user" | "assistant";
          ...
          try {
            ...
            parsed = parseWorkoutResponse(response) as NonNullable<
              typeof parsed
            >;
          } catch (error) {
            const err = error as Error;
        ...
        const streamRef =
          ...
          !Array.isArray(reference)
            ? (reference as Record<string, JsonValue>)
            : null;
        const streamPreviews = Array.isArray(previewWorkouts)
          ? previewWorkouts
          : [];
        if (streamRef && streamPreviews.length > 0 && parsed?.workouts) {
          const modifiedIndex =
          ...
          if (parsed.workouts.length === streamPreviews.length) {
            ...
            const refTitle = (streamRef.workoutTitle as string)
              ?.toLowerCase()
              .trim();
            const byTitle = parsed.workouts.find(
              (w) =>
                (w.title ?? (w as Record<string, JsonValue>).name ?? "")
                  .toString()
                  .toLowerCase()
                  .trim() === refTitle,
            );
          ...
          (parsed as Record<string, unknown>).action =
            parsed.action || "update_workout";
          (parsed as Record<string, unknown>).targetWorkoutId =
            streamRef.workoutTitle;
        }
