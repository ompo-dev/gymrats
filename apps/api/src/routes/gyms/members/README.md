...
- Caminho: `app/api/gyms/members`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/gyms/members`

## Subpastas
- `[membershipId]/`: subdomínio de `app/api/gyms/members/[membershipId]`.
...
### `route.ts`
- O que faz: implementa o endpoint `/api/gyms/members` com método(s) `métodos não detectados`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `createSafeHandler`, `getMembers`, `json`, `enrollStudent`), serializando resposta HTTP.
