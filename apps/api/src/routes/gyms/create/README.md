...
### `route.ts`
- O que faz: implementa o endpoint `/api/gyms/create` com método(s) `POST`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `POST`, `createGymHandler`), serializando resposta HTTP.
