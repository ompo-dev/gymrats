...
- Caminho: `app/api/gym-subscriptions/current`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/gym-subscriptions/current`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/gym-subscriptions/current` com método(s) `GET`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `GET`, `getCurrentGymSubscriptionHandler`), serializando resposta HTTP.
