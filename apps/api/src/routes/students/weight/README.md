...
### `route.ts`
- O que faz: implementa o endpoint `/api/students/weight` com método(s) `GET, POST`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `GET`, `getWeightHistoryHandler`, `POST`, `addWeightHandler`), serializando resposta HTTP.
