...
- Caminho: `app/api/students/day-passes`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/students/day-passes`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/students/day-passes` com método(s) `GET`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `GET`, `getDayPassesHandler`), serializando resposta HTTP.
