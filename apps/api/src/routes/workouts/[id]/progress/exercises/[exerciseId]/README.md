...
- Caminho: `app/api/workouts/[id]/progress/exercises/[exerciseId]`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/workouts/[id]/progress/exercises/[exerciseId]`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/workouts/[id]/progress/exercises/[exerciseId]` com método(s) `PUT, PATCH`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `PUT`, `resolve`, `updateWorkoutProgressExerciseHandler`, `PATCH`), serializando resposta HTTP.
