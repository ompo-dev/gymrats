...
- Caminho: `app/api/workouts/manage`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/workouts/manage`

## Subpastas
- `[id]/`: subdomínio de `app/api/workouts/manage/[id]`.
...
### `route.ts`
- O que faz: implementa o endpoint `/api/workouts/manage` com método(s) `POST`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `POST`, `createWorkoutHandler`), serializando resposta HTTP.
