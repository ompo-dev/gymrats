...
function formatProfileResponse(profile: {
  ...
  return {
    ...
    availableEquipment: parseJsonArray<string>(profile.availableEquipment),
    ...
    physicalLimitations: parseJsonArray<string>(profile.physicalLimitations),
    motorLimitations: parseJsonArray<string>(profile.motorLimitations),
    medicalConditions: parseJsonArray<string>(profile.medicalConditions),
...
export async function getAllStudentDataForUser({
  studentId,
  userId,
  sections,
  ...
  try {
    ...
    if (!requestedSections || requestedSections.includes("profile")) {
      ...
      if (student?.profile) {
        ...
        result.profile = {
          ...formatProfileResponse(student.profile),
          dietType: student.profile.dietType,
          allergies: parseJsonArray<string>(student.profile.allergies),
        ...
        return {
          ...
          exercises: wh.exercises.map((el) => {
            ...
            return {
              ...
              sets,
              notes: el.notes || null,
              formCheckScore: el.formCheckScore || null,
              difficulty:
                el.difficulty &&
                [
                  "muito-facil",
