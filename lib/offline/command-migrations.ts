...
export function migrateCommand(command: Command): Command {
	...
	for (let version = meta.version; version < currentVersion; version++) {
		migratedCommand = applyMigration(
			migratedCommand,
			type,
			version,
			version + 1,
...
function getCurrentVersion(type: string): number {
	...
	const versions: Record<string, number> = {
		...
		GYM_CHECKIN_CREATE: 1,
		GYM_CHECKOUT_UPDATE: 1,
		GYM_EXPENSE_CREATE: 1,
		GYM_PAYMENT_CREATE: 1,
		GYM_PAYMENT_STATUS_UPDATE: 1,
		GYM_MEMBERSHIP_UPDATE_STATUS: 1,
		GYM_EQUIPMENT_CREATE: 1,
		GYM_EQUIPMENT_UPDATE: 1,
		GYM_MAINTENANCE_CREATE: 1,
		GYM_PLAN_CREATE: 1,
		GYM_PLAN_UPDATE: 1,
		GYM_PLAN_DELETE: 1,
		GYM_MEMBER_ENROLL_CREATE: 1,
		GYM_SUBSCRIPTION_CREATE: 1,
		GYM_SUBSCRIPTION_CANCEL: 1,
		CUSTOM: 1,
	};
