import type { UserProfile, CalorieCalculation, CardioType } from "./types"
...
export function calculateCaloriesBurned(
  ...
  const ageMultiplier = userProfile.age > 40 ? 1 - (userProfile.age - 40) * 0.005 : 1.0
  ...
  let hormonalProfile: "testosterone-dominant" | "estrogen-dominant" | "balanced" | undefined

  if (userProfile.isTransgender && userProfile.hormoneTreatment) {
    hormonalProfile = userProfile.hormoneTreatment === "testosterone" ? "testosterone-dominant" : "estrogen-dominant"
  ...
  return {
    exercise: activity,
    type: activity.includes("corrida") || activity.includes("bicicleta") ? "cardio" : "strength",
    duration,
    intensity,
...
export function calculateCardioCalories(
  ...
  const calc = calculateCaloriesBurned(cardioType, intensity, duration, userProfile)
...
export function calculateStrengthTrainingCalories(
  ...
  const calc = calculateCaloriesBurned("musculacao", intensity, duration, userProfile)
  return calc.caloriesBurned
}
