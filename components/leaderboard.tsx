...
export function Leaderboard() {
  const getRankIcon = (rank: number) => {
    if (rank === 1)
      return (
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-duo-yellow text-xl">
          🥇
        </div>
      );
    if (rank === 2)
      return (
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-duo-gray-light text-xl">
          🥈
        </div>
      );
    if (rank === 3)
      return (
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-duo-orange/30 text-xl">
          🥉
        </div>
      );
    return (
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-duo-gray-dark">
          Compete com seus amigos pelo topo
        </p>
      </div>

      <div className="rounded-2xl border-2 border-duo-blue bg-linear-to-br from-duo-blue/10 to-duo-green/10 p-6">
        ...
        <div className="text-center text-sm text-duo-gray-dark">
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isCurrentUser
                  ? "border-duo-blue bg-duo-blue/10 shadow-lg"
                  : "border-duo-gray-border bg-white"
              )}
            >
                ...
                <div className="flex-1">
                  <div className="mb-1 flex items-center gap-2">
                    <span
                      className={cn(
                        "font-bold",
                        isCurrentUser ? "text-duo-blue" : "text-duo-text"
                      )}
                    >
                      {entry.user.name}
                    </span>
                    {entry.user.isOnline && (
                      <div
                        className="h-2 w-2 rounded-full bg-duo-green"
                        title="Online"
                      />
                    )}
                  </div>
                  <div className="text-xs text-duo-gray-dark">
                    Nível {entry.user.level} • {entry.user.currentStreak} dias
                    streak
                  </div>
                </div>
