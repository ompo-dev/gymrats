...
export function StreakModal({ open, onClose, currentStreak }: StreakModalProps) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md border-2 border-duo-border bg-white p-0">
        ...
        <div className="p-6 text-center">
          ...
          <h2 className="mb-2 text-2xl font-bold text-duo-text">{currentStreak} dia de sequência!</h2>
          <p className="mb-6 text-sm text-duo-gray-dark">Continue praticando todos os dias para manter sua sequência</p>

          {/* Calendar grid */}
          <div className="mb-6 rounded-2xl border-2 border-duo-border bg-gray-50 p-4">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-duo-gray-dark">Julho 2025</span>
            </div>

            <div className="grid grid-cols-7 gap-2">
              {weekDays.map((day, index) => (
                <div key={index} className="text-center">
                  <div className="mb-2 text-xs font-bold text-duo-gray-dark">{day}</div>
                  <div
                    className={cn(
                  ...
                  >
                    {index === today && <Flame className="h-5 w-5 fill-duo-orange text-duo-orange" />}
                  </div>
                </div>
              ))}
            </div>
