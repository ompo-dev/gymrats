...
export function AppHeader({ userType, stats, showLogo = true }: AppHeaderProps) {
  const [streakModalOpen, setStreakModalOpen] = useState(false)
  const isGym = userType === "gym"

  const primaryColor = isGym ? "#FF9600" : "#58CC02"
  const primaryColorClass = isGym ? "border-[#FF9600] text-[#FF9600]" : "border-duo-green text-duo-green"
    ...
    <>
      <header
        className={cn(
          ...
          isGym ? "border-b-2 border-gray-200 shadow-sm" : "border-b border-duo-border",
        )}
      >
        <div
          className={cn(
            "flex h-[70px] items-center px-4",
            isGym ? "justify-end gap-2 px-3" : "container justify-between",
          )}
        >
          ...
          <div className={cn("flex items-center", isGym ? "gap-2" : "gap-3")}>
            ...
            {isGym && stats.level && (
              <div className="flex items-center gap-1.5 rounded-2xl border-2 border-[#FF9600] bg-white px-3 py-1.5">
                ...
                <span className="text-xs font-bold text-[#FF9600]">Nv {stats.level}</span>
              </div>
            )}
            ...
            >
              <Flame className={cn(isGym ? "h-4 w-4" : "h-5 w-5", "fill-duo-orange text-duo-orange")} />
              <span className={cn("font-bold text-duo-orange", isGym ? "text-xs" : "text-sm")}>{stats.streak}</span>
            </button>

            {/* XP/Gems */}
            <button
              className={cn(
                "flex items-center gap-1.5 rounded-2xl border-2 border-duo-blue bg-white transition-all hover:bg-blue-50",
                isGym ? "px-3 py-1.5" : "px-4 py-2",
              )}
            >
              <div className={isGym ? "h-4 w-4" : "h-5 w-5"}>
              ...
              <span className={cn("font-bold text-duo-blue", isGym ? "text-xs" : "text-sm")}>{stats.xp}</span>
            </button>
            ...
            {isGym && stats.ranking && (
              <div className="flex items-center gap-1.5 rounded-2xl border-2 border-[#CE82FF] bg-white px-3 py-1.5">
                ...
                <span className="text-xs font-bold text-[#CE82FF]">#{stats.ranking}</span>
              </div>
            )}
          </div>
        </div>
      ...
      {!isGym && (
        <StreakModal open={streakModalOpen} onClose={() => setStreakModalOpen(false)} currentStreak={stats.streak} />
      )}
    </>
  )
}
