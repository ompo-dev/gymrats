	...
	const selectOptions: DuoSelectOption[] = [
		...gyms.map((gym) => ({
			...
			icon: gym.logo ? (
				...
				<Building2 className={cn("h-5 w-5 shrink-0", !gym.isActive ? "text-duo-gray" : "text-duo-fg")} />
			),
			badge: !gym.isActive ? (
		...
		// Se não puder criar mais, mostramos como "Locked" ou com aviso
		{
			...
			icon: <Plus className={cn("w-5 h-5", canCreateMultipleGyms ? "text-duo-green" : "text-duo-gray-dark")} />,
