...
export function WorkoutModal() {
	...
	const _loadProgress = loaders?.loadProgress;
	...
	const _addXP = async (amount: number) => {
	...
	useEffect(() => {
		...
		activeWorkout?.skippedExercises,
		activeWorkout,
		workout,
	]);
	...
	const handleFinish = async () => {
		...
		const workoutDuration = updatedWorkout?.startTime
			? Math.round(
					(Date.now() - new Date(updatedWorkout.startTime).getTime()) / 60000,
				)
			: workout.estimatedTime;
