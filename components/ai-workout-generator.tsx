...
import type { WorkoutSession, MuscleGroup, DifficultyLevel, UserProfile } from "@/lib/types"
...
export function AIWorkoutGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    const prompt = `Crie um treino de ${preferences.muscleGroup} nível ${preferences.difficulty} com duração de ${preferences.duration} minutos.
    Preferências: ${userProfile?.preferredSets || 3} séries, faixa de ${userProfile?.preferredRepRange || "hipertrofia"}, 
    descanso ${userProfile?.restTime || "medio"}.
    Equipamentos disponíveis: ${preferences.equipment.join(", ") || "academia completa"}.
    ${userProfile?.injuries ? `Lesões a evitar: ${userProfile.injuries.join(", ")}` : ""}`
  ...
  const equipment = ["Barra", "Halteres", "Máquinas", "Peso corporal", "Elásticos", "Kettlebell"]
  ...
  if (isGenerating) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center space-y-4">
        ...
        <div className="text-xl font-bold text-duo-text">IA criando seu treino...</div>
        <div className="text-sm text-duo-gray-dark">Personalizando para você</div>
      </div>
    )
  }
      ...
      <div className="space-y-6">
        <div className="rounded-2xl border-2 border-duo-green bg-gradient-to-br from-duo-green/10 to-duo-blue/10 p-6">
          <div className="mb-4 flex items-start justify-between">
            <div>
              <div className="mb-2 text-2xl font-bold text-duo-text">{generatedWorkout.title}</div>
              <div className="text-sm text-duo-gray-dark">{generatedWorkout.description}</div>
            </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="flex items-center gap-2 text-sm font-bold text-duo-gray-dark">
              <Zap className="h-4 w-4 text-duo-yellow" />+{generatedWorkout.xpReward} XP
            </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {generatedWorkout.exercises.map((exercise, index) => (
              <div key={exercise.id} className="rounded-2xl border-2 border-duo-gray-border bg-white p-4">
                <div className="mb-2 flex items-start justify-between">
                  ...
                  <div className="text-xs font-bold text-duo-gray-dark">{exercise.rest}s descanso</div>
                </div>
                {exercise.notes && (
                  <div className="mt-2 rounded-lg bg-duo-blue/10 p-2 text-xs text-duo-text">{exercise.notes}</div>
                )}
              </div>
            ))}
          </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-duo-blue to-duo-green">
          <Sparkles className="h-10 w-10 text-white" />
        </div>
        <h1 className="mb-2 text-3xl font-bold text-duo-text">Gerador de Treinos com IA</h1>
        <p className="text-sm text-duo-gray-dark">Treinos personalizados baseados no seu perfil</p>
      </div>

      <div className="space-y-4">
        <div>
          <label className="mb-2 block text-sm font-bold text-duo-gray-dark">Grupo Muscular</label>
          <div className="grid grid-cols-2 gap-2">
            {muscleGroups.map((group) => (
              <button
                key={group.value}
                onClick={() => setPreferences({ ...preferences, muscleGroup: group.value })}
        ...
        <div>
          <label className="mb-2 block text-sm font-bold text-duo-gray-dark">Nível de Dificuldade</label>
        ...
        <div>
          <label className="mb-2 block text-sm font-bold text-duo-gray-dark">Duração (minutos)</label>
          <input
            type="range"
            min="20"
            max="90"
            ...
            onChange={(e) => setPreferences({ ...preferences, duration: Number.parseInt(e.target.value) })}
            className="w-full"
          />
          <div className="mt-2 text-center text-xl font-bold text-duo-text">{preferences.duration} minutos</div>
        </div>

        <div>
          <label className="mb-2 block text-sm font-bold text-duo-gray-dark">Equipamentos Disponíveis</label>
          <div className="grid grid-cols-2 gap-2">
            {equipment.map((eq) => (
              <button
                key={eq}
