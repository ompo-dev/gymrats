...
- Caminho: `app/api/gym-subscriptions/cancel`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/gym-subscriptions/cancel`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/gym-subscriptions/cancel` com método(s) `POST`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `POST`, `cancelGymSubscriptionHandler`), serializando resposta HTTP.
