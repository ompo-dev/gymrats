...
- Caminho: `app/api/gym-subscriptions/start-trial`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/gym-subscriptions/start-trial`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/gym-subscriptions/start-trial` com método(s) `POST`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `POST`, `startGymTrialHandler`), serializando resposta HTTP.
