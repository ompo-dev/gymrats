...
export async function POST(request: NextRequest) {
  try {
    ...
    const { message, conversationHistory = [], unitId, existingWorkouts = [], profile } = body;
    ...
    if (workoutsInfo.length > 0) {
      const workoutsInfoText = workoutsInfo
        .map((w: any) => `- ${w.title} (${w.type}, ${w.muscleGroup}): ${w.exercises.length} exercícios`)
        .join('\n');
    ...
    if (student?.profile) {
      ...
      if (profileData.weeklyWorkoutFrequency) {
        profileInfo.push(`Frequência semanal: ${profileData.weeklyWorkoutFrequency} dias`);
      }
      if (profileData.workoutDuration) {
        profileInfo.push(`Duração preferida: ${profileData.workoutDuration} minutos`);
      }
      if (profileData.preferredSets) {
        profileInfo.push(`Séries preferidas: ${profileData.preferredSets}`);
      }
      if (profileData.preferredRepRange) {
        profileInfo.push(`Faixa de repetições preferida: ${profileData.preferredRepRange}`);
      }
