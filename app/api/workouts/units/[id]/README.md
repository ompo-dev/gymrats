...
- Caminho: `app/api/workouts/units/[id]`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/workouts/units/[id]`
...
### `route.ts`
- O que faz: implementa o endpoint `/api/workouts/units/[id]` com método(s) `PUT, DELETE`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `PUT`, `updateUnitHandler`, `DELETE`, `deleteUnitHandler`), serializando resposta HTTP.
