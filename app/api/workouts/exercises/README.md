...
- Caminho: `app/api/workouts/exercises`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/workouts/exercises`

## Subpastas
- `[id]/`: subdomínio de `app/api/workouts/exercises/[id]`.
...
### `route.ts`
- O que faz: implementa o endpoint `/api/workouts/exercises` com método(s) `POST`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `POST`, `createExerciseHandler`), serializando resposta HTTP.
