    ...
    switch (parsedPlan.action) {
      case "create_workouts": {
        ...
        for (const workoutPlan of parsedPlan.workouts) {
          try {
            ...
            results.created.push(`${exercises.length} exercícios em ${workoutPlan.title}`);
          } catch (error: any) {
            results.errors.push(`Erro ao criar workout ${workoutPlan.title}: ${error.message}`);
          }
        }
        break;
      }
      ...
      case "delete_workout": {
        if (parsedPlan.targetWorkoutId) {
          const workout = unit.workouts.find((w: any) => w.id === parsedPlan.targetWorkoutId);
          if (workout) {
      ...
      case "add_exercise": {
        if (parsedPlan.targetWorkoutId && parsedPlan.workouts.length > 0) {
          ...
          const targetWorkout = unit.workouts.find((w: any) => w.id === parsedPlan.targetWorkoutId);
      ...
      case "remove_exercise": {
        if (parsedPlan.targetWorkoutId && parsedPlan.exerciseToRemove) {
          const workout = unit.workouts.find((w: any) => w.id === parsedPlan.targetWorkoutId);
          if (workout) {
            const exercise = workout.exercises.find((e: any) =>
              e.name.toLowerCase().includes(parsedPlan.exerciseToRemove!.toLowerCase()) ||
              parsedPlan.exerciseToRemove!.toLowerCase().includes(e.name.toLowerCase())
            );
      ...
      case "replace_exercise": {
        ...
        if (parsedPlan.workouts.length > 1) {
          await db.workoutExercise.deleteMany({ where: { workout: { unitId } } });
        ...
        } else if (parsedPlan.targetWorkoutId && parsedPlan.exerciseToReplace && parsedPlan.workouts.length > 0) {
          const workout = unit.workouts.find((w: any) => w.id === parsedPlan.targetWorkoutId);
          if (workout) {
            const oldExercise = workout.exercises.find((e: any) =>
              e.name.toLowerCase().includes(parsedPlan.exerciseToReplace!.old.toLowerCase()) ||
              parsedPlan.exerciseToReplace!.old.toLowerCase().includes(e.name.toLowerCase())
            );
      ...
      case "update_workout": {
        if (parsedPlan.workouts.length > 1) {
        ...
        } else if (parsedPlan.targetWorkoutId && parsedPlan.workouts.length > 0) {
          // Atualizar apenas o workout referenciado (modo antigo)
          const workoutPlan = parsedPlan.workouts[0];
          
          // Buscar workout pelo ID primeiro
          let targetWorkout = unit.workouts.find((w: any) => w.id === parsedPlan.targetWorkoutId);
          ...
          if (!targetWorkout) {
            targetWorkout = unit.workouts.find((w: any) => {
              const targetTitle = parsedPlan.targetWorkoutId.toLowerCase().trim();
              const workoutTitle = w.title.toLowerCase().trim();
              return workoutTitle === targetTitle || 
          ...
          if (targetWorkout) {
            ...
            console.log(`[update_workout] Workout não encontrado pelo ID/título "${parsedPlan.targetWorkoutId}", criando novo...`);
...
async function createExercisesInBatch(
  ...
  console.log(`[createExercisesInBatch] Criando ${exercises.length} exercícios para workout ${workoutId}`);

  for (let i = 0; i < exercises.length; i++) {
    const exercisePlan = exercises[i];

    try {
      console.log(`[createExercisesInBatch] Processando exercício ${i + 1}/${exercises.length}: ${exercisePlan.name}`);

      // Buscar ou criar exercício no database educacional
      const exerciseInfo = findOrCreateExercise(exercisePlan.name);

      console.log(`[createExercisesInBatch] Exercício encontrado/criado: ${exerciseInfo.id} - ${exerciseInfo.name}`);

      // Calcular sets, reps e rest baseado nas preferências do aluno
      const calculatedSets = exercisePlan.sets ||
        calculateSets(profile.preferredSets, profile.activityLevel, profile.fitnessLevel);
      const calculatedReps = exercisePlan.reps ||
        calculateReps(profile.preferredRepRange, profile.goals ? JSON.parse(profile.goals) : []);
      const calculatedRest = exercisePlan.rest !== undefined
        ? exercisePlan.rest
    ...
    try {
      let alternativesToCreate: Array<{ name: string; reason: string; educationalId: string | null }> = [];

      // Se a IA forneceu alternativas, usar elas (prioridade)
      if (exercisePlan.alternatives && Array.isArray(exercisePlan.alternatives) && exercisePlan.alternatives.length > 0) {
        console.log(`[createExercisesInBatch] Usando ${exercisePlan.alternatives.length} alternativas fornecidas pela IA`);
        alternativesToCreate = exercisePlan.alternatives.slice(0, 3).map((altName: string) => ({
          name: altName.trim(),
        ...
        console.log(`[createExercisesInBatch] Gerando alternativas automaticamente para ${exercisePlan.name}`);
      ...
      if (alternativesToCreate.length > 0) {
        ...
        console.log(`[createExercisesInBatch] ✅ ${alternativesToCreate.length} alternativas criadas para ${exercisePlan.name}`);
      } else {
        console.warn(`[createExercisesInBatch] ⚠️ Nenhuma alternativa criada para ${exercisePlan.name} - IA deveria fornecer 2-3 alternativas`);
      }
    } catch (altError) {
      console.error('[createExercisesInBatch] Erro ao adicionar alternativas:', altError);
      // Não falhar a criação do exercício se houver erro nas alternativas
    }

    createdExercises.push(exercise);
    console.log(`[createExercisesInBatch] ✅ Exercício criado com sucesso: ${exercise.name} (ID: ${exercise.id})`);
    } catch (exerciseError: any) {
      // Log do erro mas continue criando os outros exercícios
      console.error(`[createExercisesInBatch] ❌ Erro ao criar exercício ${exercisePlan.name}:`, exerciseError);
      // Não adicionar ao array de criados, mas continuar o loop
    }
  }

  console.log(`[createExercisesInBatch] ✅ Total de ${createdExercises.length}/${exercises.length} exercícios criados com sucesso`);
