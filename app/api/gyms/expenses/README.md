...
### `route.ts`
- O que faz: implementa o endpoint `/api/gyms/expenses` com método(s) `métodos não detectados`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `createSafeHandler`, `getExpenses`, `json`, `createExpense`), serializando resposta HTTP.
