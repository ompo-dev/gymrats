...
- Caminho: `app/api/gyms/plans`
- Finalidade: raiz do App Router com páginas, layouts e endpoints por filesystem routing.
- Endpoint base deste diretorio: `/api/gyms/plans`

## Subpastas
- `[planId]/`: subdomínio de `app/api/gyms/plans/[planId]`.
...
### `route.ts`
- O que faz: implementa o endpoint `/api/gyms/plans` com método(s) `métodos não detectados`.
- Como: lê request, aplica validações/autorização e delega para handlers/casos de uso (ex.: `createSafeHandler`, `getPlans`, `json`, `createPlan`), serializando resposta HTTP.
