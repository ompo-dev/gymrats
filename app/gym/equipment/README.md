...
### `page-content.tsx`
- O que faz: implementa o módulo `page-content.tsx` da camada `equipment`.
...
- Importa principalmente: `lucide-react`, `next/link`, `react`, `@/components/molecules/relative-time`, `@/components/ui/button`, `@/components/ui/card`, `@/components/ui/input`, `@/lib/types`
- Expõe: sem exports nomeados (ou apenas default).
- Comunica com: camada local sem integrações explícitas detectadas.
- Onde é usado/importado: `app/gym/equipment/page.tsx`

### `page.tsx`
- O que faz: implementa a página de entrada da rota `app/gym/equipment/page.tsx`.
- Como: compõe componentes e hooks para executar o fluxo principal da tela; operações detectadas: `EquipmentPage`, `getGymEquipment`.
