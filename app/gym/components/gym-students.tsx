  ...
  return (
    <div className="p-4 md:p-8">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-black text-gray-900 md:text-4xl">Gestão de Alunos</h1>
              ...
              <Card className="group cursor-pointer border-2 p-6 transition-all hover:border-[#58CC02] hover:shadow-lg">
                <div className="mb-4 flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#58CC02]">{student.name}</h3>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between rounded-xl border-2 p-3">
                    <div className="flex items-center gap-2">
                      <Flame className={`h-5 w-5 ${getStreakColor(student.currentStreak)} fill-current`} />
                      <span className="font-bold text-gray-700">Sequência</span>
                    </div>
                    <span className={`text-xl font-black ${getStreakColor(student.currentStreak)}`}>
                  ...
                  <div className="rounded-xl border-2 p-3">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <span className="text-xl font-black text-gray-900">{student.attendanceRate}%</span>
                    </div>
                    <div className="h-2 overflow-hidden rounded-full bg-gray-200">
                      <div
                        className={`h-full ${getAttendanceColor(student.attendanceRate)} transition-all`}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl bg-[#1CB0F6]/10 p-3 text-center">
                      <p className="text-2xl font-black text-[#1CB0F6]">{student.totalVisits}</p>
                      <p className="text-xs font-bold text-gray-600">Treinos</p>
                    </div>
                    <div className="rounded-xl bg-[#CE82FF]/10 p-3 text-center">
                      <p className="text-2xl font-black text-[#CE82FF]">{student.currentWeight}kg</p>
                ...
                {student.assignedTrainer && (
                  <div className="mt-4 rounded-xl bg-gray-100 p-2 text-center">
                    <p className="text-xs font-bold text-gray-600">
                      Personal: <span className="text-gray-900">{student.assignedTrainer}</span>
                    </p>
                  </div>
                )}
              </Card>
        ...
        {filteredStudents.length === 0 && (
          <Card className="border-2 p-12 text-center">
            <p className="text-xl font-bold text-gray-500">Nenhum aluno encontrado</p>
