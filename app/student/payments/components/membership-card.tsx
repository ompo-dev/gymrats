...
export interface MembershipCardProps {
	...
	changePlanPlans: Array<{ id: string; name: string; type: string; price: number; duration: number }>;
	onToggleExpand: () => void;
	onTrocarPlano: () => void;
						...
						<div className="mt-3 flex flex-wrap items-center gap-2">
							<span
								className={cn(
									...
									membership.status === "active" && "bg-duo-green/20 text-duo-green",
									membership.status === "suspended" && "bg-duo-orange/20 text-duo-orange",
									membership.status === "canceled" && "bg-duo-red/20 text-duo-red",
									membership.status === "pending" && "bg-duo-yellow/20 text-duo-yellow",
								)}
							>
							...
							<div className="min-w-0 text-right">
								...
								<p className="text-sm font-bold text-duo-text mt-0.5">
									{membership.nextBillingDate
										? new Date(membership.nextBillingDate).toLocaleDateString("pt-BR")
										: "N/A"}
								</p>
							</div>
						</div>
