...
export async function getStudentProfileData() {
	try {
		...
		const [profile, progress, workoutHistory, personalRecords, weightData] = await Promise.all([
		...
		const ranking = await StudentProgressService.getRanking(studentId, progress.totalXP);
		...
		return {
			...
			userInfo: {
				...
				memberSince: new Intl.DateTimeFormat("pt-BR", { month: "short", year: "numeric" }).format(ctx.user.createdAt),
			},
