	...
	"inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-2xl font-bold text-base transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-offset-2 uppercase tracking-wide shadow-lg active:scale-[0.98]",
	{
		variants: {
			variant: {
				...
				blue: "bg-[#1CB0F6] text-white hover:bg-[#1A9CE0] focus-visible:ring-[#1CB0F6] shadow-[#1CB0F6]/30",
				orange:
				...
				red: "bg-[#FF4B4B] text-white hover:bg-[#E63939] focus-visible:ring-[#FF4B4B] shadow-[#FF4B4B]/30",
				ghost:
	...
	(
		...
		if (asChild) {
			return (
				<Slot
					className={cn(
						duoButtonVariants({ variant, size, animation, className }),
					)}
					{...(props as any)}
				>
					{children}
