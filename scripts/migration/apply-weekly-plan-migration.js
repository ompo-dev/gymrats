...
async function applySchemaMigration() {
	console.log("📐 Aplicando schema (weekOverride, weekly_plans, plan_slots)...\n");
	...
	const fkSteps = [
		['weekly_plans_studentId_fkey', 'weekly_plans', 'studentId', 'students', 'id'],
		['plan_slots_weeklyPlanId_fkey', 'plan_slots', 'weeklyPlanId', 'weekly_plans', 'id'],
...
async function applyDataMigration() {
	try {
		...
		for (const student of students) {
			...
			const title =
				student.units[0]?.title || "Meu Plano Semanal";
			...
			const workoutCount = slotsToCreate.filter((s) => s.type === "workout").length;
