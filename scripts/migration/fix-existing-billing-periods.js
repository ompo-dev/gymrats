...
async function fixExistingSubs() {
  ...
  for (const sub of subs) {
    let period = null;
    if (sub.plan.toLowerCase().includes('anual') || sub.plan.toLowerCase().includes('annual')) {
        period = 'annual';
    } else if (sub.plan.toLowerCase().includes('mensal') || sub.plan.toLowerCase().includes('monthly')) {
        period = 'monthly';
    }
