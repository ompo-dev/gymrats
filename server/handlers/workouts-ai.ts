...
export async function generateWorkoutsHandler({
  ...
  try {
    ...
    if (!student || !student.profile) {
      ...
      return {
        success: false,
        error: "Perfil do aluno não encontrado. Complete o onboarding primeiro.",
      };
    }
    ...
    return internalErrorResponse(set, "Erro ao gerar treinos personalizados", error);
  }
}
...
export async function populateEducationalDataHandler({ set }: WorkoutsAiContext) {
  try {
...
export async function processWorkoutsCommandHandler({
  ...
  try {
    ...
    if (unit.studentId !== studentId) {
      return badRequestResponse(set, "Você não tem permissão para editar esta unit");
    }
    ...
    switch (parsedPlan.action) {
      ...
      default:
        return badRequestResponse(set, `Ação não suportada: ${parsedPlan.action}`);
    }
  ...
  try {
    ...
    if (student?.profile) {
      ...
      if (profileData.restTime) {
        profileInfo.push(`Tempo de descanso preferido: ${profileData.restTime}`);
      }
...
export async function chatStreamWorkoutsHandler({
  ...
  const stream = new ReadableStream({
    ...
    "exercises": [/* MODIFICAR exercícios conforme pedido do usuário, mantendo estrutura de alternatives */]
                  ...
                  const exercisesJson = JSON.stringify(w.exercises || [], null, 2)
                    .split("\n")
...
- Você DEVE usar targetWorkoutId="${workoutIdentifier}"`;
        ...
        if (!parsed) {
          ...
          try {
            ...
            sendSSE("error", { error: error.message || "Erro ao processar mensagem" });
            controller.close();
            return;
          }
        }
        ...
        if (
          ...
          const modifiedIndex =
            typeof reference.workoutIndex === "number" ? reference.workoutIndex : 0;
  ...
  if (!exerciseInfo) {
    ...
    const inferMuscleGroup = (name: string): string[] => {
      ...
      const rules: Array<{ muscles: string[]; keywords: string[] }> = [
        ...
        {
          muscles: ["bracos"],
          keywords: ["triceps", "tríceps", "pulley", "biceps", "bíceps", "rosca"],
        },
    ...
    exerciseInfo = {
      ...
      tips: ["Mantenha a forma correta", "Controle o movimento", "Use amplitude completa"],
      commonMistakes: [
