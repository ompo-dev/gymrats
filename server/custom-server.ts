...
function applyResponseHeaders(
  ...
  const getSetCookie = (response.headers as { getSetCookie?: () => string[] })
    .getSetCookie;
...
async function handleApiRequest(
  ...
  if (response.status === 404) {
    const text = await response
      ...
      .text()
      .catch(() => "");
    if (text === "NOT_FOUND" || text === "Not Found" || text.trim() === "") {
      return handleNextFallback(req, res);
    }
  }
...
const elysiaRoutes = apiApp.routes.map((r) => ({
  method: r.method,
  regex: new RegExp(
...
function isElysiaRoute(method: string, pathname: string) {
  return elysiaRoutes.some(
    (r) =>
      (r.method === method || r.method === "ALL") && r.regex.test(pathname),
  );
}
...
nextApp.prepare().then(() => {
  const server = createServer(async (req, res) => {
    ...
    if (pathname && (pathname.startsWith("/api") || pathname === "/health")) {
      if (isElysiaRoute(req.method || "GET", pathname)) {
        await handleApiRequest(req, res, handleNext);
        return;
      }
    }
