...
export async function signUpUseCase(
  ...
  if (existingUser) {
    return fail("Este email ja esta cadastrado", 400);
  }
...
export async function forgotPasswordUseCase(
  ...
  return ok({
    message:
      "Se o email estiver cadastrado, voce recebera um codigo de verificacao.",
  });
}
...
export async function verifyResetCodeUseCase(
  ...
  if (!verificationToken) {
    return fail("Codigo invalido", 400);
  }
  ...
  if (deps.now() > verificationToken.expires) {
    ...
    return fail("Codigo expirado. Solicite um novo codigo.", 400);
  }
