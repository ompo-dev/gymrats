 ...
 */
export async function addToQueue(item: Omit<OfflineQueueItem, 'id' | 'timestamp' | 'retries'>): Promise<string> {
 ...
 */
export async function moveToFailed(item: OfflineQueueItem, error: string): Promise<void> {
  const db = await getDB();
