...
export async function logCommand(command: Command): Promise<void> {
  try {
    ...
    const store = db.transaction('commands', 'readwrite').objectStore('commands');
    ...
    await store.put({
      ...
      command: {
        ...
        errorDetails: command.error ? {
          ...
          stack: command.error instanceof Error ? command.error.stack : undefined,
        } : undefined,
      },
      loggedAt: Date.now(),
    });
...
export async function getCommandsByStatus(
  ...
  try {
    const db = await getLogDB();
    const store = db.transaction('commands', 'readonly').objectStore('commands');
 ...
 */
export async function getRecentCommands(limit: number = 50): Promise<Command[]> {
  try {
    const db = await getLogDB();
    const store = db.transaction('commands', 'readonly').objectStore('commands');
    const index = store.index('logged');
