...
import { Dumbbell, ChevronDown, MessageSquare } from "lucide-react";
...
interface WorkoutPreviewCardProps {
  ...
  index: number;
  onReference?: (type: "workout" | "exercise", workoutIndex: number, exerciseIndex?: number) => void;
}
...
function ExerciseItemCard({
  ...
  const hasDetails = exercise.notes || (exercise.alternatives && exercise.alternatives.length > 0);
...
export function WorkoutPreviewCard({ workout, index, onReference }: WorkoutPreviewCardProps) {
    ...
    >
      <DuoCard
        ...
        className={cn(
          ...
          hasExercises && "cursor-pointer hover:border-duo-green/50 active:scale-[0.98]"
        )}
        onClick={hasExercises ? () => setIsExpanded(!isExpanded) : undefined}
      >
