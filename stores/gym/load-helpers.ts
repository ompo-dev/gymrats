...
> {
	return members.map((m) => {
		...
		const status = m.status ?? (m as { membershipStatus?: string }).membershipStatus ?? "active";
		...
		return {
			...
			profile: {
				...
				weeklyWorkoutFrequency:
					(profile.weeklyWorkoutFrequency as number) ?? 0,
			},
			progress: {
				currentStreak: (progress.currentStreak as number) ?? 0,
	...
	const promise = (async () => {
		try {
			...
			return transformSectionResponse(
				section,
				response.data as Record<string, import("@/lib/types/api-error").JsonValue>,
			);
		} catch (error) {
